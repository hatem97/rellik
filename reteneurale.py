...
class risultatoReteNeurale(Resource):
    def get(self,v1,p1,s1,r1, squadra1, squadra2):
        data = pd.DataFrame({
            'v1': dati.data[:, 0],
            'p1': dati.data[:, 1],
            's1': dati.data[:, 2],
        ...
        predictions = mlp.predict_proba([[v1, p1, s1, r1,0,0,0,0]])
...
api.add_resource(risultatoReteNeurale, '/risultatoReteNeurale/<v1>&<p1>&<s1>&<r1>&<squadra1>&<squadra2>') # Route_2
